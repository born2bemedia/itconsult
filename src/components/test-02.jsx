...
function RequestForm() {
  ...
  // Кастомный компонент Select для Formik
  const CustomSelect = ({ field, form, options, ...props }) => {
    const { setValue, setTouched } = form;

    const handleChange = (selectedOption) => {
      setValue(field.name, selectedOption ? selectedOption.value : "");
      setTouched({ ...form.touched, [field.name]: true });
    };
    ...
    return (
      <div className="row">
        <Select
          ...
          className={
            form.touched[field.name] && form.errors[field.name] ? "invalid" : ""
          }
